...
for (i in tenk$X10k_link){
  ...
  #thirteen = str_extract(string = twelve, pattern = "(?s)(?i)°Item 1A.*?°Item")
  #a = str_extract(string = twelve, pattern = "(?s)(?i)°Item 7A.*?°Item")
  ...
  #text = paste(thirteen,fourteen, fifteen, sep=" ")
    tenk_text = append(tenk_text, twelve)
    count = count + 1
    print(count)
...
```{r}
second_half = tenk[1001:1239, ]

second_half$tenk_data = tenk_text[1:239]
```

```{r}
third_half = tenk[1241: 1458, ]
third_half$tenk_data = tenk_text[241:458]
...
```{r}
write.csv(second_half, file = 'train_second_half.csv')
write.csv(third_half, file = 'train_third_half.csv')
```
